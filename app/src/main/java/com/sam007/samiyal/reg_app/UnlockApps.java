...
public class UnlockApps extends ListActivity {

    private Integer counter = 0;

    private PackageManager packageManager = null;
    private List<ApplicationInfo> applist = null;
    private UnlockApplicationAdapter listadaptor = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_unlock_apps);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot i : dataSnapshot.getChildren()) {
                    for(DataSnapshot j: i.getChildren()) {
                        ...
                        try {
                            ainfo = getPackageManager().getApplicationInfo(temp, 0);
                            if(ainfo != null) {
                                Log.d("lasttry",ainfo.packageName.toString());
                                applist.add(ainfo);
                            }
            ...
            try {
                if (null != packageManager.getLaunchIntentForPackage(info.packageName)) {
                    for(String i: locklist) {
                        if(i.toLowerCase().equals(info.packageName.toLowerCase())) {
                            applist.add(info);
                        }
                    }
                }
    ...
    private class LoadApplications extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            listadaptor = new UnlockApplicationAdapter(getApplicationContext(),
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        ...
        try {
            ...
            AlertDialog.Builder setLockDialog = new AlertDialog.Builder(UnlockApps.this);
            LayoutInflater inflater = LayoutInflater.from(UnlockApps.this);
            final View myview = inflater.inflate(R.layout.set_unlock,null);
            setLockDialog.setView(myview);
            final AlertDialog dialog = setLockDialog.create();

            final EditText test = myview.findViewById(R.id.u_pass);
            Button go = myview.findViewById(R.id.u_btn);
            Button cnl = myview.findViewById(R.id.u_cnl);
                        ...
                        mAuth.signInWithEmailAndPassword(email,pass).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                            ...
                            public void onComplete(@NonNull Task<AuthResult> task) {
                                if(task.isSuccessful()) {
                                    final String apptounlock = p_name;
                                    ...
                                    mRef.addListenerForSingleValueEvent(new ValueEventListener() {
                                        ...
                                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                            for(DataSnapshot i : dataSnapshot.getChildren()) {
                                                for(DataSnapshot j: i.getChildren()) {
                                                    String temp = j.getValue().toString();
                                                    Log.d("unlockfinal",temp +" == "+ apptounlock);
                                                    if(temp.equals(apptounlock)) {
                                                        Log.d("unlockmate",temp +" == "+ apptounlock);
                                                        mRef.child(uid).child(j.getKey()).removeValue();
                                                        startActivity(new Intent(getApplicationContext(),UnlockApp.class));
